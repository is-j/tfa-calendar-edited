if (accountType == 'tutor') {
    ...
    document.getElementById('informationForm').addEventListener('submit', function (event) {
        ...
        checkValidity(form, event, () => {
            const meetingLink = form.querySelector('input[name="meeting_link"]').value;
            const bio = form.querySelector('textarea[name="bio"]').value;
            const languages = [];
            for (checkbox of form.querySelectorAll('input[type="checkbox"]')) {
                if (checkbox.checked) {
                    languages.push(parseInt(checkbox.getAttribute('data-lang')));
                }
            }
            form.querySelector('fieldset').setAttribute('disabled', true);
            form.querySelector('button').setAttribute('disabled', true);
            form.querySelector('button').innerHTML = '<svg class="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"> <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle> <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path> </svg> Updating...';
            postData('/api/information/update', {
                ...
                form.querySelector('button').innerHTML = 'Update information';
            }, 1000));
        });
    });
}